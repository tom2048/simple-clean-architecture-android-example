...
class UsersRepositoryMemory(private val appSchedulers: AppSchedulers) : UsersRepository {
    ...
    private val users = mutableMapOf<String, UserWithCredentials>().apply {
        // Test users
        put("a312b3ee-84c2-11eb-8dcd-0242ac130003", UserWithCredentials(UserDetails("a312b3ee-84c2-11eb-8dcd-0242ac130003", "Nickname1", "nickname1@test.com", "Test description 1")))
        put("3b04aacf-4320-48bb-8171-af512aae0894", UserWithCredentials(UserDetails("3b04aacf-4320-48bb-8171-af512aae0894", "Nickname2", "nickname2@test.com", "Test description 2")))
        put("52408bc4-4cdf-49ef-ac54-364bfde3fbf0", UserWithCredentials(UserDetails("52408bc4-4cdf-49ef-ac54-364bfde3fbf0", "Nickname3", "nickname3@test.com", "Test description 3")))
    }

    override fun getList(): Observable<List<UserDetails>> {
        synchronized(users) {
            return Observable.timer(TEST_DELAY_SECONDS, TimeUnit.SECONDS)
    ...
    override fun get(id: String): Observable<UserDetails> {
        synchronized(users) {
            return users[id]?.let {
                val user = it.user
                Observable.timer(TEST_DELAY_SECONDS, TimeUnit.SECONDS)
                    .map { user }
                    .subscribeOn(appSchedulers.io())
            } ?: run {
                Observable.timer(TEST_DELAY_SECONDS, TimeUnit.SECONDS)
    ...
    override fun insert(user: UserDetails): Single<String> {
        synchronized(users) {
            return if (user.id.isNullOrEmpty()) {
                ...
                Single.timer(TEST_DELAY_SECONDS, TimeUnit.SECONDS)
    ...
    override fun update(user: UserDetails): Completable {
        synchronized(users) {
            return user.id?.let { id ->
                ...
                Completable.timer(TEST_DELAY_SECONDS, TimeUnit.SECONDS)
                    .subscribeOn(appSchedulers.io())
            } ?: run {
                Completable.timer(TEST_DELAY_SECONDS, TimeUnit.SECONDS)
    ...
    companion object {
        private const val TEST_DELAY_SECONDS = 1L
    }
